...
(define (get-component-name component-id)
  (hash-table-ref/default *component-name-map* component-id "unnamed-component"))
...
(export transform-entity-component)
...
(export maybe-do-with-component)
...
(export call-with-system-components)
...
(export entity-has-been-deleted-from-system?)
...
(export system-filter-map-entities)
...
(define-record link prev next)
(define-record system components components-order first last enter exit pre post every)
...
(define-record entity id components system-links)
